...
class PIDController:
    def __init__(self, expert_ckpt_path="checkpoints/maniskill_ppo_expert.pt"):
...
def main():
    ...
    parser.add_argument("--wm_ckpt_root", type=str, default="checkpoints/wm/maniskill",
                        help="Root directory of the DINO-WM checkpoints (final dir is <root>/<backbone>)")
    parser.add_argument("--hj_ckpt_dir", type=str, default="checkpoints/hj/maniskill",
                        help="Root directory of the trained HJ checkpoints")
    parser.add_argument("--output_dir", type=str, default="close_loop",
                        help="Directory to save rollout videos and summaries")
    parser.add_argument("--expert_ckpt", type=str, default="checkpoints/maniskill_ppo_expert.pt",
    ...
    if backbone == "full_scratch":
        wm_ckpt_dir = f"{args.wm_ckpt_root}/vc1"
    else:
        wm_ckpt_dir = f"{args.wm_ckpt_root}/{backbone}"
    if finetune:
        backbone += "_ft"
    hj_ckpt_dir = f"{args.hj_ckpt_dir}/{backbone}/latest"
    if args.pid_only:
        video_save_path = f"{args.output_dir}/maniskill_{args.eps}/pid/"
    else:
        video_save_path = f"{args.output_dir}/maniskill_{args.eps}/{'dynamics' if args.only_dynamics else 'critic'}/{backbone}"
    ...
    # Create the eval env before the evaluator runs any encoder inference:
    # creating a SAPIEN env after dinov2 inference segfaults on some GPU setups.
    env = ManiskillEnvForTesting(device)
    pid_controller = PIDController(expert_ckpt_path=args.expert_ckpt)
    ...
    for use_dynamics in dynamics_settings:
        ...
        for mode in modes:
            ...
            # Reuse the eval env created in main (fresh SAPIEN envs segfault
            # if created after encoder inference on some GPU setups)

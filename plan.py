...
### load checkpoint with weights_only=False because torch/serialization.py error

def load_ckpt_not_weights_only(snapshot_path, device):
    with snapshot_path.open("rb") as f:
...
def load_model_not_weights_only(model_ckpt, train_cfg, num_action_repeat, device, ):
    result = {}
    if model_ckpt.exists():
        result = load_ckpt_not_weights_only(model_ckpt, device)
        print(f"Resuming from epoch {result['epoch']}: {model_ckpt}")

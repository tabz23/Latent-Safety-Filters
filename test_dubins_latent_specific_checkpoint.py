...
def main():
    ...
    parser.add_argument("--wm_ckpt_root", type=str, default="checkpoints/wm/dubins",
                        help="Root directory of the DINO-WM checkpoints (final dir is <root>/<backbone>)")
    parser.add_argument("--hj_ckpt_dir", type=str, default="checkpoints/hj/dubins",
    ...
    if finetune:
        backbone += "_ft"
    hj_ckpt_dir = f"{args.hj_ckpt_dir}/{backbone}/epoch_200"
    if args.pid_only:
        video_save_path = f"{args.output_dir}/dubins_{args.eps}/pid/"
    else:
        video_save_path = f"{args.output_dir}/dubins_{args.eps}/{'dynamics' if args.only_dynamics else 'critic'}/{backbone}"

...
#     parser.add_argument('--output-dir', type = str, default = 'datasets/maniskill')
...
def collect_episodes(name, num_episodes, max_steps, obs_mode, output_dir, expert_ckpt="checkpoints/maniskill_ppo_expert.pt"):
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    obses_dir = output_path / "obses"
    obses_dir.mkdir(exist_ok=True)
...
def main():
    ...
    parser.add_argument('--output-dir', type = str, default = 'datasets/maniskillnew')
    ...
    collect_episodes(args.name, args.num_episodes, args.max_steps, args.obs_mode, args.output_dir, args.expert_ckpt)
